...
def get_args():
    ...
    parser.add_argument("--input", type=str, default="input/input.mp4", help="Path to input video")
    ...
    parser.add_argument("--stride", type=int, default=10, help="size of each component image")
...
def main(opt):
    ...
    while cap.isOpened():
        ...
        for i, j in product(range(int(width / opt.stride)), range(int(height/opt.stride))):
            ...
            blank_image[j * opt.stride: (j + 1) * opt.stride,
                        i * opt.stride: (i + 1) * opt.stride, :] = images[idx]
        if opt.overlay_ratio:
            overlay = cv2.resize(frame, (int(width * opt.overlay_ratio), (int(height * opt.overlay_ratio))))
            blank_image[height - int(height * opt.overlay_ratio):,
                        width - int(width * opt.overlay_ratio):, :] = overlay
